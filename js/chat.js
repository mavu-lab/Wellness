function helperLabel(h) {
  return {
    elder: "Mother/Elder — warmth & forgiveness",
    money: "Money Elder — healthy family not excess",
    health: "Health — body kindness, 1% less",
    growth: "Growth Friend — are you growing?",
    peer: "Peer Coach — gossip changed face",
    healer: "Healer — fix vs let go, scar"
  }[h] || h;
}
...
async function sendChat() {
  ...
  // Record current user interaction to local storage arrays immediately
  ...
  try {
    ...
    // Slice previous logs EXCLUDING your newly saved active message turn (preventing payload duplicates)
    const baselineHistory = Store.getChats().slice(0, -1).slice(-6);
    const data = await getChat(text, baselineHistory);
...
async function loadModSuggestions(lastUserMsg) {
  try {
    ...
    data.suggestions.forEach((s, i) => {
      ...
      btn.innerText = `${i + 1}. ${s}`; 
      btn.onclick = () => { 
        document.getElementById('modInput').value = s; 
        document.getElementById('modInput').dataset.helper = data.helper; 
      };
      box.appendChild(btn);
    });
  } catch (e) {}
