...
async function captureThought() {
  ...
  const totalRaws = Store.getRaws().length;
  document.getElementById('mirrorText').innerText = `Captured ${totalRaws} raws. Mirroring...`;
  updateGrowth();

  try {
    const { getMirror } = await import('./ai.js');
    
    // Extract array items natively instead of stringifying the collection into a joined block
    const recentRawsArray = Store.getRaws()
      .map(r => r.text || "")
      .filter(Boolean)
      .slice(-10); // Capture the last 10 clean text nodes to safely fit the API parameter caps

    const data = await getMirror(recentRawsArray);
    
    document.getElementById('mirrorText').innerText = data.mirror;
    document.getElementById('mirrorText').dataset.helper = data.helper;
  } catch (e) {
    document.getElementById('mirrorText').innerText = `Captured. Raw, blurred, human. Life goes on. — ${totalRaws} thoughts held locally.`;
  }
}
...
function updateGrowth() {
  const r = Store.getRaws().length; 
  const f = Store.getFutures().length;
  const el = document.getElementById('growth'); if (!el) return;
  
  if (r === 0) {
    el.innerText = "Start: write one raw thought, no filter.";
  } else {
    el.innerText = `Life Goes On: ${r} raws, ${f} future letters. You are growing, not just loyal.`;
  }
}
